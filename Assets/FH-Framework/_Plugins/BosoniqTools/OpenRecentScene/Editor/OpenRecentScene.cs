...
// Copyright (c) 2016 Bosoniq (tools@bosoniq.com)
// </copyright>
//---------------------------------------------------------------------------
...
namespace Bosoniq.Tools
{
	...
	{
		public static readonly System.Version Version = new System.Version("1.4.0");
		...
		private static void OnPlaymodeStateChanged()
		{
				...
				&& (!string.IsNullOrEmpty(SceneRestoreOnPlayStop)))
			{
				...
				var scenePath = SceneRestoreOnPlayStop;
		...
		protected static void OnSceneChanged()
		{
			...
			if (!string.IsNullOrEmpty(currentProject))
			{
				...
				var scenePaths = projectScenes[currentProject];

				// Place the current scene at the top of the recent-usage list.
				var isListChanged = false;
				if (!string.IsNullOrEmpty(currentScene))
				{
					var currentSceneIndex = scenePaths.IndexOf(currentScene);
				...
				for (var sceneIndex = (scenePaths.Count - 1); sceneIndex >= 0; sceneIndex--)
				{
					var scene = scenePaths[sceneIndex];
			...
			bool includeEntryForCurrentProject = false)
		{
			...
			var savedData = EditorPrefs.GetString(ScenePathsPreference);
			var data = ((!string.IsNullOrEmpty(savedData)) ?
				savedData.Split(PreferenceDataSeparator) : null);
			var dataCount = ((data != null) ? data.Length : 0);
			var dataIndex = 0;
			while (dataIndex < dataCount)
			{
				var projectPath = data[dataIndex++];
				...
				int sceneCount;
				var isParseSuccess = int.TryParse(
					data[dataIndex++],
			...
			bool discardOldProjects = true)
		{
			if (discardOldProjects)
			{
				...
				var currentProject = CurrentProject;
				foreach (var projectPath in projectScenes.Keys.ToArray())
				{
			...
			var projectData = projectScenes.
				...
				ToArray();
			var data = string.Join(separator, projectData);
			...
			bool includeEntryForCurrentProject = false)
		{
			...
			var savedData = EditorPrefs.GetString(SceneRestorePreference);
			var data = ((!string.IsNullOrEmpty(savedData)) ?
				savedData.Split(PreferenceDataSeparator) : null);
			var dataCount = ((data != null) ? data.Length : 0);
			var dataIndex = 0;
			...
			{
				long timestamp;
				var isParseSuccess = long.TryParse(
					data[dataIndex++],
					System.Globalization.NumberStyles.Integer,
					InvariantCultureFormat,
					out timestamp);
			...
			bool discardOldData = true)
		{
			if (discardOldData)
			{
				...
				var timeNow = GetSceneRestoreTimestampAtPresent();
			...
			var separator =
				PreferenceDataSeparator.ToString(InvariantCultureFormat);
			var restoreData = sceneRestoreData.
				Select(item => string.Format(
					...
					item.Value.Value)).
				ToArray();
			var data = string.Join(separator, restoreData);
		...
		{
			var timestampUtc = System.DateTime.UtcNow;
		...
		{
			var indexLabelStyle = this.indexLabelStyle;
			...
			{
				var sceneButtonStyle = this.GetSceneButtonStyle();
		...
		{
			var highlightButtonStyle = this.highlightButtonStyle;
		...
		{
			var sceneButtonStyle = this.sceneButtonStyle;
		...
		protected virtual void OnGUI()
		{
			...
			var isCompleteSceneSearchRequested = false;
			...
			if (currentEvent.type == EventType.keyDown)
			{
				...
				{
					var eventModifiers = this.GetCurrentEventModifiers();
			...
			var displayMessage = this.displayMessage;
			...
			this.scrollPosition = GUILayout.BeginScrollView(this.scrollPosition);
			{
				GUILayout.BeginVertical();
				{
					var highlightButtonStyle = this.GetHighlightButtonStyle();
					var scenePathsMaxIndex = (scenePaths.Count - 1);
					...
					{
						var sceneButtonStyle = this.GetSceneButtonStyle();
						var indexLabelStyle = this.GetIndexLabelStyle(scenePathsMaxIndex);
						GUILayout.BeginHorizontal();
						{
							GUILayout.Space(2);
							GUILayout.BeginVertical();
							{
								for (var sceneIndex = 0; sceneIndex <= scenePathsMaxIndex; sceneIndex++)
								{
		...
		{
			var backgroundTexture = this.GetBackgroundTexture();
		...
		protected virtual Texture GetBackgroundTexture()
		{
			...
			if (!backgroundTexture)
			{
				...
				for (int x = 0, width = backgroundTexture.width; x < width; x++)
				{
					...
					{
						var greyscale = (((x - y + height) / lineScale) % 1.0f);
						greyscale = Mathf.Abs((greyscale * 2.0f) - 1.0f);
						var color = new Color(greyscale, greyscale, greyscale, alpha);
		...
		protected virtual void AddExtraScenePaths(bool isFullSearchRequested)
		{
			var scenePaths = this.cachedScenePaths;
			var currentProject = CurrentProject;
			var currentProjectLength = currentProject.Length;
			...
			{
				foreach (var scenePath in previousCachedScenePaths)
				{
				...
				|| (scenePaths.Count <= 0))
			{
				...
				var allSceneFiles = Directory.GetFiles(
					CurrentAssets,
					("*" + SceneFileExtension),
				...
				{
					foreach (var scenePath in allSceneFiles)
					{
						addScenePathIfMissing(scenePath);
					}
				}
			...
			string scenePath, EventModifiers eventModifiers)
		{
			if (!string.IsNullOrEmpty(scenePath))
			{
				...
				var isModeAdditive = ((eventModifiers & (EventModifiers.Control | EventModifiers.Command)) != 0);
				var isPlayRequest = ((eventModifiers & EventModifiers.Alt) != 0);
				...
				switch (eventModifiers)
				{
					...
					case EventModifiers.Command:
						...
						else
						{
							...
							var i = selectionList.IndexOf(sceneAsset);
							if (i < 0)
							{
								// Select the scene.
		...
		{
			var title = ("About Open Recent Scene v." + Version);
			const string Bullet = "\u2022\u00A0";
			var message =
			...
			else
			{
				...
				if (sceneCount > 0)
				{
					...
					else
					{
						...
						while (true)
						{
							var trimLength = sceneTexts[0].IndexOf(PathSeparator);
							if (trimLength <= 0)
							{
								break;
							}

							var trimText = sceneTexts[0].Substring(0, trimLength);
					...
					&& (movedFromAssetPaths.Length == movedAssets.Length))
				{
					...
					if (isSceneInvolved)
					{
						...
						if (!string.IsNullOrEmpty(currentProject))
						{
							...
							var isListChanged = false;
							for (var i = (sceneIndices.Length - 1); i >= 0; i--)
							{
								var sceneRank =
